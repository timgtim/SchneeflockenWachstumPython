...
def durchschnitt(index, zeilen,  länge_Liste, beta, l_Zellen):
...
def rezeptiv(index, zeilen, länge_Liste, l_Zellen):
...
def main(zeilen, alpha, beta, gamma):
    ...
    while loopW:
        ...
        for i in l_Zellen[(zeilen-1)*zeilen:]: #Überprüfen, ob Schneeflocke Rand von Simulation erreicht hat
            if i >= 1:
                loopW = False
...
def mainFürGif(zeilen, alpha, beta, gamma, gifAbstand=5):
    ...
    while loopW:
        ...
        for i in l_Zellen[(zeilen-1)*zeilen:]: #Überprüfen, ob Schneeflocke Rand von Simulation erreicht hat
            if i >= 1:
                loopW = False
        if 0 <= kern_Position + gifAbstandDynamisch*zeilen < len(l_Zellen) and l_Zellen[kern_Position + gifAbstandDynamisch*zeilen] >= 1:
            if l_Zellen[kern_Position + gifAbstandDynamisch*zeilen] >= 1 and (kern_Position + gifAbstandDynamisch*zeilen)<(länge_Liste-1):
